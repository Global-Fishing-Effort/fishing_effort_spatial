...
for(spatial_type in spatial_scales){
...
# scale_max <- max(abs(cmip_data_plot$mean), na.rm = TRUE)
# scale_mid <- max(abs(cmip_data_plot$mean), na.rm = TRUE)/2
# 
# scale_label_max <- glue("{round(scale_max)}%")
# scale_label_mid <- glue("{ceiling(scale_mid)}%")
...
# rank_rgn_adj$end_point <- scale_max # point where we will want to place the country labels
...
rgn_shift <- rgn_shift %>%
  ...
  rgn_shift <- data.frame(
                         contnnt = rank_pos_neg$contnnt,
                         name_x = c(20, 75, 125, 165, 200), # 140
...
rank_rgn_adj$uncertain <- factor(rank_rgn_adj$uncertain, c("yes", "no"))
...
## categorical fill - pattern for uncertainty
  ...
  geom_errorbar(
   ...
   geom_segment(
    ...
    annotate("text",
       ...
       #  geom_image(x = 0, y = -unique(rank_rgn_adj$end_point)/4, image = here("fishmip/images/FishMIP_logo.png"), size = 0.15) + # for fishmip logo
  ...
  ggsave(glue(here("fishmip/figs/bar_{scen}_{decade_type}_{spatial_type}_pattern.png")), height=20, width=20, units=c("in"), bg = "white")
  ...
  ggsave(glue(here("fishmip/figs/bar_{scen}_{decade_type}_{spatial_type}.png")), height=20, width=20, units=c("in"), bg = "white")
...
ggsave(plot = g_legend, here("fishmip/figs/legend.png"), dpi = 300, height = 8, width =8)

```

Put the scenarios into a panel... this doesn't work so well. I think I will combine them in some sort of image editor instead. Doing it in R doesn't really retain the resolution of the plot well. 
...
#Loading images and making grobs

plots <- lapply(ll <- list.files(path = here("fishmip/figs/"), pattern = 'countries_high_seas.png', full.names = TRUE),function(x){
    img <- as.raster(readPNG(x))
    rasterGrob(img, interpolate = FALSE)
})

ggsave(here("fishmip/figs/Plots_Combined.png"),width=8.5, height=11, 
...
decades <- c("2091-2100", "2041-2050")
scenarios <- c("ssp585", "ssp126")
eez_shp <- vect(here("fishmip/data/"), crs = "+proj=moll +R=10567000 +lon_0=0 +x_0=0 +y_0=0 +units=m +towgs84=0,0,0,0,0,0,0 +no_defs") 
eez_key <- read.csv(here("fishmip/data/FAO-Major-Areas_EEZ_continent_keys.csv")) %>%
  mutate(figure_name = ifelse(!is.na(NAME_EN), NAME_EN, figure_name))
...
ggsave(glue(here("fishmip/figs/map_{scen}_{decade_type}.png")), bg = "white")
