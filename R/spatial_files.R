...
writeRaster(raster_empty, here("data/raw/spatial/empty_rast_0.1.tif"), overwrite = TRUE)
...
# Create an empty SpatRaster with 0.01-degree resolution
raster_empty <- rast(
  ncol = 36000,  # 360 degrees / 0.01 degrees = 36000 columns
  nrow = 18000,  # 180 degrees / 0.01 degrees = 18000 rows
  xmin = xmin, xmax = xmax, 
  ymin = ymin, ymax = ymax
)
...
writeRaster(raster_empty, here("data/raw/spatial/empty_rast_0.01.tif"), overwrite = TRUE)
...
write.csv(test, here("data/raw/spatial/eez_lookup.csv"), row.names = FALSE)

## get regions IDs

## rasterize to 0.1 by 0.1, using MRGID column (this is the ID for territory EEZs)

raster_empty <- rast(here("data/raw/spatial/empty_rast_0.1.tif"))
...
writeRaster(eez_rast, here("data/raw/spatial/eez_id_rast_0.1.tif"), overwrite = TRUE)
...
qs::qsave(eez_lookup, here("data/raw/spatial/eez_rast_csv_0.1.qs"))
...
## rasterize to 0.01 by 0.01, using MRGID column (this is the ID for territory EEZs)

raster_empty <- rast(here("data/raw/spatial/empty_rast_0.01.tif"))


eez_rast <- rast(fasterize::fasterize(eezs, raster(raster_empty), field = "MRGID"))
eez_rast

eez_rast[is.na(eez_rast)] <- 99999 # save a value high seas, not if rousseau has that in there..
eez_rast <- mask(eez_rast, vect(world_shp), inverse = TRUE) # mask out any land 
eez_rast[is.na(eez_rast)] <- 999999 # save a value for land, not sure if rousseau has that in there..


writeRaster(eez_rast, here("data/raw/spatial/eez_id_rast_0.01.tif"), overwrite = TRUE)


# eez_rast <- rast(here("data/raw/spatial/eez_id_rast_0.01.tif"))
...
eez_lookup_new <- eez_lookup %>%
  # filter(layer != 999999) %>% ## just filter out land values so we save space when saving 
  rename(mrgid = layer) %>%
  mutate(x = ifelse(x > 0, ceiling(x * 100) / 100, floor(x * 100) / 100),
         y = ifelse(y > 0, ceiling(y * 100) / 100, floor(y * 100) / 100))

# test <- eez_lookup_new %>% 
#   filter(mrgid != 999999)

eez_check <- eez_lookup_new %>%
  mutate(x_int = floor(x), y_int = floor(y)) %>%  # extract integer part of x and y
  group_by(x_int, y_int) %>%
  summarise(x_unique_decimals = n_distinct(x), 
            y_unique_decimals = n_distinct(y)) %>%
...
# Check for any integers that do not have exactly 10 decimal versions
missing_decimals <- eez_check %>%
  filter(x_unique_decimals != 100 | y_unique_decimals != 100) 
test <- eez_lookup_new %>%
  filter(is.na(mrgid)) # ok no NAs in mrgid that's good

# has a shitload of cells...

test_flk <- eez_lookup %>%
  filter(mrgid == 8389)

qs::qsave(eez_lookup_new, here("data/raw/spatial/eez_rast_csv_0.01.qs"))
## every point i've looked up seems right... 