...
# eez_rgns_old <- read.csv(here("int/eez_raster_df.csv"))

eez_rgns <- qread(here("raw/spatial/eez_rast_csv.qs"))
eez_lookup <- read.csv(here("raw/spatial/eez_lookup.csv")) %>%
  clean_names()
...
gfw_fix_props <- gfw_fix %>%
  ...
  left_join(eez_lookup) %>%
    mutate(flag_gfw = ifelse(flag_gfw == ""|flag_gfw == "UNK", iso_sov1, flag_gfw)) %>%
  mutate(flag_gfw_name = countrycode(sourcevar = flag_gfw, origin = "iso3c", destination = "country.name")) %>%
  mutate(flag_gfw = ifelse(flag_gfw %in% c("HSX", "LND"), "UNK", flag_gfw),
         flag_gfw_name = ifelse(flag_gfw %in% c("UNK"), "Unknown", flag_gfw_name)) %>% # %>% # ok these cells are all either on land or in the high seas and don't report the flag country... so we just assume they are unknown unfortunately. 
  # filter(fishing_hours > 0) # if you filter out 0 fishing hours, there are no NAs. I'll keep 0's in for now
  dplyr::select(-mrgid_sov1, -iso_ter1, -geoname, -hours_global, -fishing_hours_global, -nv_global, -engine_power_kw_gfw_global, -tonnage_gt_gfw_global) %>%
  dplyr::select(cell_ll_lon, cell_ll_lat, everything())

# test_na <- gfw_fix_props %>%
#     filter(is.na(flag_gfw_name)) # these are all 0 for 2012
# 
# test_0 <- gfw_fix_props %>%
#   filter(fishing_hours == 0)
# 
# test_na <- gfw_fix_props %>%
#     filter(is.na(mrgid) & is.na(flag_gfw_name))

qs::qsave(gfw_fix_props, file.path(rdsi_dir, glue("prep/prorate_allocation/gfw_effort/all_effort_gear_length_props_{y}.qs")))
...
test <- qs::qread(file.path(rdsi_dir, "prep/prorate_allocation/gfw_effort/all_effort_gear_length_props_2012.qs"))
