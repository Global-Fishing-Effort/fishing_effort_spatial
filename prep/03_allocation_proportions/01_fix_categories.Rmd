...
Now we need to reconcile the gfw data categories to those of the Rousseau categories. 
 - Some of the gears have different names.
    - Because of this, we need to split the "trawlers" category from GFW based on proportion of bottom vs midwater trawls reported in Rousseau et al. 
    - fixed gear should be lumped into an "Other" category
    - I think we will just need to group the Rouuseau and GFW data into a general "Other" category
 - GFW reports length as a numeric but Rousseau is category

Get country level trawling proportions from Rousseau et al:
...
qs::qsave(trawling_eff, here("int/prop_trawling_year_country.qs")) # save 
...
qsave(rousseau_gear_fix, here("int/rousseau_gear_fix.qs"))

## start with length, that is easy


sort(unique(rousseau_gear_fix$gear_new))
...
qs::qsave(gfw_fix_fin, here("int/gfw_vessel_registry_fix.qs")) # now save this file so we can read back in later and join to gfw data
