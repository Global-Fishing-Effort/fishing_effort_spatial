...
wind_hist_raw <- rast(list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), full.names = TRUE)) # shit.. its 0.5 by 0.5 degree... need to aggregate to 1x1 
...
raw_files <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), full.names = TRUE)
...
## save
writeRaster(wind_hist_ocean, glue(file.path(rdsi_raw_dir, "ISIMIP/wind/{file_name}_ocean.tif")))
...
hist_files <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), pattern = "_ocean", full.names = TRUE)[5:7]
for(file in hist_files[1:2]){
 # file <- hist_files[1]
...
hist_files <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), pattern = "_ocean", full.names = TRUE)

for(file in hist_files[2:7]){
 #  file = hist_files[1]
  ...
  writeRaster(ocean_hist_wind_bias_agg, file.path(rdsi_raw_dir, glue("ISIMIP/wind/{filename}_bias_1.tif")))
}


## now calculate yearly averages and resample to our raster format save rasters for those years 

hist_files <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), pattern = "_ocean_bias_1", full.names = TRUE)
for(file in hist_files[1:7]){
 # file <- hist_files[1]
  ocean_hist_wind <- rast(file)
...
old_wind <- read.csv(here("data/model_features/deg_1_x_1/remss_wind.csv"))
hist_files_mean <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), pattern = "bias_corrected_1", full.names = TRUE)[1:7]
hist_files_sd <- list.files(file.path(rdsi_raw_dir, "ISIMIP/wind/"), pattern = "bias_corrected_1", full.names = TRUE)[8:14]
