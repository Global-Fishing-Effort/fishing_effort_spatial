...
wrangle_pdo_data <- 
  read.table(url("https://www.ncei.noaa.gov/pub/data/cmb/ersst/v5/index/ersst.v5.pdo.dat"), skip = 2) %>%
    ...
    group_by(year) %>%
  summarise(mean = mean(pdo_index, na.rm = TRUE),
            sd = sd(pdo_index, na.rm = TRUE)) %>%
  ungroup() #
...
N. Rayner et al., Global analyses of sea surface temperature, sea ice, and night marine air temperature since the late nineteenth century. J. Geophys. Res. Atmos. 108, 1–25 (2003).
