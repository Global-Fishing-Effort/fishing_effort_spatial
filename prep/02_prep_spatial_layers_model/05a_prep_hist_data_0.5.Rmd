...
# there are some cells that are missing, so we will fill those in
...
sst_1950_1980 <- sst_data_imputed %>%
  filter(year %in% c(1950:1980))

sst_1981_2014 <- sst_data_imputed %>%
  filter(year %in% c(1981:2014))
...
chl_1950_1980 <- chl_data_imputed %>%
  filter(year %in% c(1950:1980))

chl_1981_2014 <- chl_data_imputed %>%
  filter(year %in% c(1981:2014))
...
nrow(wind_data_imputed) == nrow(wind_data_full) # TRUE
  ...
  qs::qsave(wind_data_imputed, here("data/int/prediction_historical_data/half_degree/wind_yearly_1950_2014.qs")) # wonder why its smaller than the sst and chl? 
