...
rousseau_eezs <- sort(unique(hist_fish_data$flag_fin)) #151
...
write.csv(eez_lookup_fix, here("data/model_features/deg_1_x_1/eez/eez_lookup.csv"), row.names = FALSE)
  ...
  eez_sovereign_nations <- eez %>%
    filter(POL_TYPE !=  "Overlapping claim") %>%
    filter(!str_detect(GEONAME, "Greenland|Faeroe|Cook Islands|Guadeloupe|French Guiana|French Polynesia|New Caledonia|Réunion|Wallis and Futuna|Azores|Madeira")) %>%
...
eez_data <- read.csv(here("data/model_features/deg_1_x_1/eez/eez.csv"))
...
eez_rousseau_hkg <- eez_rousseau_raw %>%
  ...
  mutate(lat = ifelse(fa_oname == "Hong Kong", 22, lat)) %>% # use the next closest latitude
  left_join(data_grid) %>%
...
write.csv(eez_lookup_fix, here("data/model_features/deg_1_x_1/eez/eez_lookup_fix.csv"), row.names = FALSE)

eez_data_fix <- eez_data %>% 
  filter(!(pixel_id == 30598 & eez_id == 8603)) %>%
  rbind(eez_rousseau_hkg)


write.csv(eez_data_fix, here("data/model_features/deg_1_x_1/eez/eez_fix.csv"), row.names = FALSE)
