---
title: "Save data for publishing and analysis"
...
```{r}
# predictions_industrial_flag_revised_floor_depth

  modelled_flags_original <- str_before_first(str_after_last(list.files(file.path(rdsi_dir, "prep/random_forest/predictions_industrial_flag_revised_floor_depth/"), pattern = ".qs", full.names = TRUE), "_"), "\\.qs")
...
foreach(flag = all_flags, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  ...
  all_files_flag_original <- list.files(file.path(rdsi_dir, "prep/random_forest/predictions_industrial_flag_revised_floor_depth/"), pattern = flag, full.names = TRUE)
...
sum(gapfill_df$regional_model)/sum(all_data_grouped$nom_active_fishing_hours) # 0.0007769694 less than 1% of total effort

gapfill_df %>% filter(year == 2017) %>% pull(regional_model) %>% sum()/sum(all_data_grouped$nom_active_fishing_hours)

# look at gapfilled countries percentage across all years
gapfill_df <- all_data_grouped %>%
  group_by(flag_country_name, model_type) %>%
  summarise(eff = sum(nom_active_fishing_days, na.rm = TRUE)) %>% 
              ungroup() %>%
  pivot_wider(names_from = "model_type", values_from = eff) %>%
  janitor::clean_names() %>%
mutate(across(everything(), ~ ifelse(is.na(.), 0, .))) %>%
  filter(regional_model > 0)
