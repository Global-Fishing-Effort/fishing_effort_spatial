...
full_model <- qs::qread(here("data/output/FULL_rf_model_train.qs"))

varImpPlot(full_model)

importance <- data.frame(importance(full_model)) %>%
  arrange(-MeanDecreaseGini)
...
test_preds <- test %>%
  mutate(pred_presence = predict(full_model, newdata = ., type="prob")[, "1"]) %>%
  # filter(gear == "Trawl_Midwater_or_Unsp",
  #        length_category == "12-24m", 
  #        year == 2015) %>% 
  filter(year == 2017) %>%
  dplyr::select(lon, lat, pred_presence) %>%
   filter(pred_presence > 0.1) %>%
  #mutate(pred_presence = ifelse(pred_presence > 0.5, 1, pred_presence)) %>%
  distinct() %>%
  rast(., type = "xyz")

plot(test_preds) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121
...
plot(bottom_trawls) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121
...
mid_trawls <- test %>%
  ...
  filter(year == 2017) %>%
  dplyr::select(lon, lat, pred_presence) %>%
   filter(pred_presence > 0.5) %>%
  distinct() %>%
  rast(., type = "xyz")

plot(mid_trawls) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121
...
plot(dredges) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121
...
plot(gillnets) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121

Lines_Longlines <- test %>%
  mutate(pred_presence = predict(full_model, newdata = ., type="prob")[, "1"]) %>%
...
plot(Lines_Longlines) # ok looks really similar to figure 1 from mcdonald et al. which it should! https://www.pnas.org/doi/10.1073/pnas.2400592121


```

## Run logistic regression models in parallel 
