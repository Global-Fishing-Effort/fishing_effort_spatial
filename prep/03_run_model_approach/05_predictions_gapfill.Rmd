...
stage1_preds <- full_data %>%
  ...
  left_join(eez_lookup) %>%
   # dplyr::select(lon, lat, flag_fin, gear, length_category, year, fao_id, eez_id, eez_sovereign, pred_presence) %>% 
...
# now rescale predictions so that if there are any length/gear categories that have no preds > 0.5, if the preds are > 0 and in their EEZ, then make them 1. 
  ...
  mutate(pred_presence_rescale = ifelse(flag_fin == eez_sovereign & pred_presence_access > 0, 1, pred_presence_rescale)) # make any predictions > 0 when flag country and EEZ match to be presence = 1, even if less than 0.5. This works, but still doesn't cover all bases for CRI for example. Maybe for the combinations we are still missing, predict only by gear type and/or length category? So we would make the same predictions for midwater trawls >50m and 24-50m? 
