...
```{r}
 ...
 modelled_flags_original <- str_before_first(str_after_last(list.files(file.path(rdsi_dir, "prep/random_forest/predictions_industrial_flag_revised_floor_0/"), pattern = ".qs", full.names = TRUE), "_"), "\\.qs")
  ...
  all_files_flag <- list.files(file.path(rdsi_dir, "prep/random_forest/predictions_industrial_flag_revised_floor_0/"), full.names = TRUE)
...
flags_missing <- unique(missing_data$flag_country_iso3c) # 19 flags missing
...
years_missing <- unique(missing_data$year) # all years missing, because of DOM for example

regions_missing_df_2 <- hist_fish_data %>%
  filter(flag_fin %in% c(flags_missing)) %>%
  distinct(flag_fin, year)

## now read get sample size based flags that we need to fill. Any flag with <100 observations will be fully gapfilled, so we need to run those models here. 

sample_size_flags <- model_data %>%
  group_by(flag_fin) %>%
  summarise(n_obs = n()) %>%
  ungroup() %>%
  filter(flag_fin %in% unique(hist_fish_data$flag_fin)) %>%
  filter(n_obs < 100) %>%
  pull(flag_fin) %>%
  unique()

flags_missing_fin <- unique(c(flags_missing, sample_size_flags))
...
setdiff(flags_missing_fin, old_missing) # 0, ok we dont need to rerun then 

test <- hist_fish_data %>%
  filter(flag_fin %in% c(flags_missing_fin)) %>%
  pull(flag_country_name) %>%
  unique()
...
```{r}
reach <- fread(here("data/int/reachability_regional_gf.csv"))

dist_eez <- fread(here("data/int/dist_from_home_eez.csv")) %>%
  dplyr::select(-eez_id) %>%
  rename(flag_fin = eez_sovereign)
```
...
flags_finished <- unique(str_after_last(str_before_last(list.files(file.path(rdsi_dir, "prep/random_forest/predictions_regional/yearly/")), "_"), "_"))
  
flags_to_run <- sort(setdiff(flags_missing_fin, flags_finished))
...
doParallel::registerDoParallel(cl)
 ...
 foreach(flag = flags_to_run, .packages = c("dplyr", "tidyverse", "randomForest", "qs", "glue", "ranger")) %dopar% {
   ...
   regional_name <- hist_fish_data %>%
     filter(flag_fin == flag) %>%
     pull(region) %>%
     unique()
   
   if(flag == "STP"){
     regional_name = "SubAfrica"
   }
...
matching_files <- list.files(
  ...
  pattern = glue::glue("stage_2_rf_model_full_data_{regional_name}_.*\\.qs$"),
  full.names = TRUE
)
...
matching_files <- list.files(
  ...
  pattern = glue::glue("stage_1_rf_model_full_data_{regional_name}_.*\\.qs$"),
  full.names = TRUE
)
...
foreach(flag = flags_missing_fin, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  # flag = "UKR"
...
```{r}
  ...
  all_files_flag <- list.files(file.path(rdsi_dir, "prep/random_forest/predictions_regional/"), pattern = ".qs", full.names = TRUE)

test <- qread(file.path(rdsi_dir, "prep/random_forest/predictions_regional/model_preds_1950_2017_GAB.qs"))

test_r <- test %>%
  filter(year == 2017) %>%
  group_by(lon, lat) %>%
  summarise(effort = sum(nom_active_fishing_days, na.rm = TRUE)) %>%
  ungroup() %>%
  rast(., type = "xyz")
...
access_ben <- access_data_loop %>%
  filter(year == 2017,
         flag_fin == "COG")

test <- qread(file.path(rdsi_dir, "prep/random_forest/predictions/model_preds_1950_2017_COG.qs"))
