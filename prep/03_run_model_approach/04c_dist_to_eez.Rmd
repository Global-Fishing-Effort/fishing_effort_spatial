...
Prep a layer which describes the distance of each cell from each home EEZ for the countries which need to use regional models for. Those countries are (you can determine these countries below)
 [1] "DOM" "RAA" "RAM" "MYT" "BEN" "BGD" "BHS" "BRN" "COD" "COG" "CUB" "DJI" "GLP" "GMB" "GNB" "GNQ" "GUF" "GUY" "JAM" "KWT" "LBN" "LCA" "MDG" "MNE" "ROU"
[26] "SUR" "SVN" "SYR" "TGO" "TON" "WLF"
...
```{r}
 ...
 modelled_flags_original <- str_before_first(str_after_last(list.files(file.path(rdsi_dir, "prep/random_forest/predictions_flag_revised_no_limit/"), pattern = ".qs", full.names = TRUE), "_"), "\\.qs")

  all_files_flag <- list.files(file.path(rdsi_dir, "prep/random_forest/predictions_flag_revised_no_limit/"), full.names = TRUE)
