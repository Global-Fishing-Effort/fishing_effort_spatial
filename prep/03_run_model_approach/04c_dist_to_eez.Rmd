---
title: "Calculate distance to each EEZ for regional models"
...
Prep a layer which describes the distance of each cell from each home EEZ for the countries which need to use regional models for. Those countries are (you can determine these countries below)
 [1] "BEN" "BGD" "BHS" "BRN" "COD" "COG" "CUB" "DJI" "DOM" "GAB" "GLP" "GMB" "GNB" "GNQ" "GTM" "GUF" "GUY" "IRQ" "JAM" "KWT" "LBN" "LCA" "MDG" "MNE" "MYT"
[26] "RAA" "RAM" "ROU" "STP" "SUR" "SVN" "SYR" "TGO" "TON" "WLF"
...
```{r}
regions <- fread(here("data/int/larger_regions_iso3c.csv")) %>%
  rename(flag_fin = Country)
...
flags_missing <- unique(missing_data$flag_country_iso3c) # 4 flags missing
...
```{r}
eez_to_calculate <- flags_missing_fin
...
unique(res_long$eez_id) # 31 good
test <- res_long %>%
  filter(eez_id == 2139) %>%
...
```{r}
library(doParallel)
# i think we will need to use the sentinel-2 data since it has speed info
...
foreach(file = sentinel_files[1:13],
...
test <- fread( here("data/int/transit_speed/transit_speed_by_eez.csv"))
test <- fread(here("data/int/transit_speed/transit_speed_global.csv"))
test <- fread( here("data/int/transit_speed/transit_speed_by_region.csv"))
```
...
test <- fread(here("data/int/transit_prop/f_transit_region_length_all_years.csv"))
...
Now we will calculate our reachability layer for the 35 countries that require regional gapfilling
