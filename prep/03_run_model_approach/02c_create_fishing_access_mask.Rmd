...
ukr_rousseau_access <- all_data_matched_effort %>% 
  ...
  ungroup() %>%
  dplyr::select(year, flag_fin = Country, eez_sovereign, NomActiveHours) %>%
  filter(flag_fin == "UKR",
         NomActiveHours > 100) %>% # maybe filter for >100 hours when we have to use this? 
...
all_catch_access <- rbind(catch_access_fin, catch_access_future, final_data) %>%
  distinct()
...
catch_access_df_highseas <- all_data_matched_catch %>%
  ...
  distinct(year, flag_fin, fao_id, fao_area_name = name_en, eez_sovereign, access_fao)
...
all_catch_access_hs <- rbind(catch_access_df_highseas_fin, catch_access_hs_future, final_data) %>%
  distinct()
