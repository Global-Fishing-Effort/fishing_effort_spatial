...
catch_access_df <- catch_df %>%
  ...
  filter(catch > 1, 
        Year >= 1950,
        SAUP != 999) %>%
  distinct(year = Year, SAUP, FCountryName, eez_country_name) %>% # maybe add gear here? Vessel length too? For as much detail as we can get? 
...
qs::qsave(catch_access_df_highseas, here("data/int/prediction_historical_data/catch_fishing_access_high_seas.qs"))
