...
vessel_info <- read.csv(file.path(rdsi_raw_dir, "global_fishing_watch/fishing-vessels-v3.csv")) %>%
  distinct(flag_gfw) # check to make sure they actually don't exist in GFW

flags <- setdiff(flags, missing_flags) # 147 flags to run now

flags_to_run <- setdiff(flags, "GNB") # add any flags we've already run
...
foreach(flag = flags_to_run, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc")) %dopar% {
