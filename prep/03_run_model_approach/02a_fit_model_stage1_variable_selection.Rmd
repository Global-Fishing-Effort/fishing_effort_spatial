...
## Read in environment and effort data  
...
## Run models and variable selection for every individual country
 ...
 - We loop through this until the AUC does not improve at all. 
  - NOTE: We keep year as a variable no matter what, because we care about the differences over time. 
...
```{r}
# ~45 mins with 24 cores
flags <- unique(hist_fish_data$flag_fin)

missing_flags <- setdiff(unique(hist_fish_data$flag_fin), unique(model_data$flag_fin))
length(missing_flags) #  #  "DOM" "MYT" "RAA" "RAM"; 4 these are flags that are in the rousseau data but not in the gfw data, meaning we can't fit a model or make predictions on them.

flags <- setdiff(flags, missing_flags) # 147 flags to run now

# done_flags <- unique(str_after_last(str_before_last(list.files(file.path(rdsi_dir, "prep/random_forest/stage_1_models/pruning/")), "_"), "_"))
...
## lets try the threshold based selection, where the threshold is 10% of max importance score (i.e. we keep any variable that is above that 10% of max importance score) and rerun the model and test to see if RMSE improves or not. BUT KEEP YEAR IN NO MATTER WHAT
...
foreach(flag = flags, .packages = c("dplyr", "tidyr", "randomForest", "qs", "glue", "tictoc", "ranger", "PRROC")) %dopar% {
  ...
  if(!file.exists((glue(file.path(rdsi_dir, "prep/random_forest/stage_1_models/pruning/stage_1_rf_train_{flag}_29.qs"))))){
  ...
  model <- ranger(
    ...
    model <- qs::qread(glue(file.path(rdsi_dir, "prep/random_forest/stage_1_models/pruning/stage_1_rf_train_{flag}_29.qs")))
}
...
Now write code to select the "best" model per the variable selection from above. We will rerun the model with just those variables on the FULL dataset to leverage all of the data from GFW and save
 - Around 1 hour with 30 cores and 100 trees
 - XX hours with 500 trees
...
stage_1_flags <- unique(c("CHN", "USA", stage_1_flags)) # move CHN and USA to the front of the list since they take the longest to run...
...
expanded_formula <- as.formula(
  ...
  tic()
    model <- ranger(
        ...
        num.trees = 500, # change this to 500 trees
        importance = "impurity",
        classification = TRUE,
        probability = TRUE,
        write.forest = TRUE
