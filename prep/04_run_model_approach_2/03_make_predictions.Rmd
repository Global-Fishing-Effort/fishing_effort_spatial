...
We use the GAM model fit in the previous script to predict the proportion of fishing effort occuring in each cell per our categorical variables (gear, length, flag country). To do this, we create a dummy dataset where any flag country can theoretically fish in any cell, and predict for that. In theory, the model should predict values to each cell where fishing is most likely.. 
...
# Function to make predictions for a specific combination
 ...
 for(flag in unique(imas_data$flag_fin)){
   ...
   if(!(flag %in% flag_levels_in_model)){
  ...
  # ok need to loop through these somehow? 
  ...
  predicted_props <- mgcv::predict.gam(model, newdata = pred_df[chunk_indices,], type = "response") # need to make sure at the end the predicted props add to one? 
  ...
  imas_data_ind <-   qs::qread(here("data/int/rousseau_gear_fix.qs")) %>%
   ...
   # filter(year == yr, flag_fin == flag) %>%
    ...
    test <- all_predictions_df %>%
      ...
      rast(., type = "xyz") # LOL ok so this obviously didn't work. The predictions are horrible. 

    
    # test <- normalized_predictions %>%
    #  # filter(gear == "Gillnets", length_category == "24-50m") %>%
    ...
    ## I tested with flag USA, dredges, 12-24m and this looks odd because there is very little data in GFW to learn from... I think
    # ok nevermind, it seems like they all are predicting basically the same??? Tried with CHN, bottom trawling and it looks horrible. 
    ...
    ## ok so we have a problem for when there is NO (or very little) GFW data for a category that is in IMAS data. In these cases, the predictions will likely be really bad, because there is no data to inform the model... how do we deal with this? 
    # 
