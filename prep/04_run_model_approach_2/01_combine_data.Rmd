...
test <- meso_data %>% 
  group_by(pixel_id) %>%
  summarise(count = n_distinct(meso_region)) # ok, that's the problem. The meso regions have pixels which have been given multiple regions. We need to fix that. 
  # right_join(grid)
...
for(yr in years) {
  ...
  year_data <- effort_props %>%
    ...
    left_join(total_efforts, by = c("flag_fin", "year", "gear", "length_category")) %>% # there are gaps here.. e.g., CHN doesn't have any vessels >50m in the Rousseau data, but does in GFW data - I actually don't think this matters though since we are allocating within these grouping represented in the Rousseau data. 
    left_join(spatial_data, by = c("x" = "lon", "y" = "lat")) %>% # why are there some NAs for the spatial data? Land values in GFW. Figure out how to deal with this. 
