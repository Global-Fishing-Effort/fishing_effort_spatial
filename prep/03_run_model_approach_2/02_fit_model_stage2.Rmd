...
percentile_cutoff <- 10  # Start with bottom 10% of variables

threshold_i <- quantile(var_imp_i[, 2], probs = percentile_cutoff / 100) # Define 10% quantile importance threshold 
# should we keep year in no matter what? Maybe? 
...
while (TRUE) {
  ...
  if (length(selected_vars) < 5) break  # Avoid over-pruning. We can change this to any number of variables. Maybe 10 would be better computationally? 
  ...
  threshold_i <- quantile(var_imp_i[, 2], probs = percentile_cutoff / 100) # calculate new threshold since variable importance metric will change? 
  ...
  qs::qsave(model_i, glue(file.path(rdsi_dir, "prep/random_forest/stage_2_models/pruning/stage_2_rf_train_{flag}_{n_vars}.qs")))


  # Check RMSE stability: Stop if improvement is <1%
  if (length(rmse_values) > 1) {
    ...
    if (rmse_improvement < 0.01) break  # Stop if RMSE stabilizes

    while(sum(var_imp_i[, 2] > threshold_i) == length(var_imp_i[, 2])) {
      threshold_i <- threshold_i*1.1 # increase by 1% until the threshold will remove a variable
    }
...
rmse_values
# ok so for CHN, that means we keep 23 variables. Whatever the next to lowest number is in the file structure for that 

```

Now write code to select the "best" model per the variable selection form above. We will rerun the model with just those variables on the FULL dataset to leverage all of the data from GFW
