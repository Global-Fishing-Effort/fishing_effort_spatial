...
all_preds <- oos_preds %>%
  ...
  mutate(pred_prop_final = pred_prop*pred_presence*access) %>%
 ...
 ungroup() %>% # need to rescale the predictions to be between 0 and 1 here
  ...
  left_join(eez_lookup) %>%
    dplyr::select(pixel_id, lon, lat, year, flag_fin, gear, length_category, pixel_area_m2, eez_sovereign, fao_id, fao_major_fishing_area,  total_fishing_hours, prop_fishing_hours_cell_predict_rescaled, nom_active_fishing_hours) %>%
