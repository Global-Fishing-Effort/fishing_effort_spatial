...
get_historical_total_fishing_hours <- function(yr) {
  ...
  imas_data <- qs::qread(here("data/int/rousseau_gear_fix.qs")) %>%
    ...
    summarize(
      total_fishing_hours = sum(nom_active_hours, na.rm = TRUE),
      .groups = 'drop'
    ) %>%
    filter(year == yr) %>%
...
# preds_path <- file.path(rdsi_dir, "prep/random_forest/predictions/")
# preds_files <- list.files(preds_path, full.names = TRUE)
# # preds_done_flags <- unique(sub(".*model_preds_([A-Z]{3}).*", "\\1", preds_files)) # 118 flags; weird there are more here than in stage 2?!
# 
# model_flags <- setdiff(model_flags, preds_done_flags)
...
all_preds <- oos_preds %>%
 ...
 group_by(year, flag_fin, flag_country_name, gear, length_category) %>%
 ...
 ungroup() %>% # need to rescale the predictions to be between 0 and 1 here
  mutate(nom_active_fishing_hours = prop_fishing_hours_cell_predict_rescaled*total_fishing_hours) %>%
  left_join(global_grid) %>%
  left_join(fao_lookup) %>%
  left_join(eez_lookup) %>%
    dplyr::select(pixel_id, lon, lat, year, flag_fin, flag_country_name, gear, length_category, pixel_area_m2, eez_sovereign, eez_country_name, fao_id, fao_major_fishing_area,  total_fishing_hours, prop_fishing_hours_cell_raw = prop_fishing_hours_cell, prop_fishing_hours_cell_predict_rescaled, nom_active_fishing_hours) %>%
...
#   ungroup() %>%
#   left_join(hist_fish_data, by = c("flag_fin", "year", "gear", "length_category"))
#
...
qs::qsave(all_preds, file.path(rdsi_dir, glue("prep/random_forest/predictions/model_preds_{flag}.qs"))) # qs is smaller
# write_parquet(all_preds, file.path(rdsi_dir, glue("prep/random_forest/predictions/model_preds_{flag}.parquet")))
}
