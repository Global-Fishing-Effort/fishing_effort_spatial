...
# there are some flags for which no predictions were made that should've been... check Honduras 2017; WLF, CIV, among others
# check what is happening to taiwan? Should be much more dispersed than is

years <- c(1950:2017)

cl <- makeCluster(10)  # could probably increase this? 
...
all_preds <- oos_preds %>%
  ...
  mutate(access = ifelse(is.na(access), 0, access),
         access_fao = ifelse(is.na(access_fao), 0, access_fao)) %>%
  mutate(access_fin = ifelse(eez_country_name == "High seas", access_fao, access)) %>%
...
foreach(flag = model_flags, .packages = c("qs", "dplyr", "tidyverse", "glue", "countrycode")) %dopar% {
  ...
  write.csv(all_data_flag, file.path(rdsi_dir, glue("prep/random_forest/zenodo_data/by_flag_country/model_preds_1950_2017_{flag}.csv")), row.names = FALSE)
}
