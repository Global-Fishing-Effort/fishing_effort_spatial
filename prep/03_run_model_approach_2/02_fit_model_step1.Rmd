...
model_formula <- formula(
  presence ~ 
   ...
   # flag_fin + 
    ...
    factor(meso_id) +  factor(eez_id) + factor(fao_id) + factor(ocean) +  # Spatial categorical variables
...
unique(hist_fish_data$length_category) # length cat does not mean "artisanal"
unique(model_data$length_category) # same cat here. 
...
### Also the opposite is problematic - i.e. combo missing in Rousseau. It is not worth modelling these as we don't have country data to spread spatially. 
...
# 497 combo in AIS that are missing from R dataset. create ID and remove? 
...
## CN are these countries that have not run from previous iterations? is it because of 
## Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
  # contrasts can be applied only to factors with 2 or more levels 
# i.e. only one gear or one length cat? Ask Gege
...
foreach(flag = flags, .packages = c("dplyr", "glue", "stats", "tictoc", "tidyr")) %dopar% {
  ...
  # CN check one of the missing combo ---- 
  # check info for midwater trawler - i.e missing_combo in CIV - longline 24-50 taken as baseline? 
  trial<- full_data %>% 
    filter(flag_fin == "CIV") %>% 
    select(gear,length_category) %>% 
    distinct()
  
  ## different set of data - does it make sense to fit a model using this if we consider rousseau as our starting data to spread? 
  trial2<- hist_fish_data %>% 
    filter(flag_fin == "CIV") %>% 
    select(gear,length_category) %>% 
    distinct()
  
  
  # need to figure out how this plays out in predictions. should add 0s as in no fishing effort for these missing combo? would then prediction be 0s or underestimates compared to Rousseau?  
  ...
  # CMR has 1 length cat only. 
  # when not in loop, the function below does not work so need to pick country manually to test the loop. 
  
  
  
  ## CN diagnostics - which? 
  plot(model)
