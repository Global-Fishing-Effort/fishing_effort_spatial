...
### GLM ---- 
tic()
model <- stats::glm(model_formula_simpler, data = full_data, family = binomial(link = "logit"))
toc() # 
model
plot(model) # awful except qq plot
...
# example https://www.r-bloggers.com/2021/04/random-forest-in-r/
library(datasets)
...
# my data - JUST understanding the function ... 
data_random_forest<-full_data %>% 
  filter(!is.na(sst_c_mean), year == 2015) %>% # cannot handle NAs - also why are there NAs? is this land? # considering only 1 year to speed up calculations 
  mutate(presence = as.factor(presence)) %>% 
  select(presence, gear, length_category, sst_c_mean)

data_random_forest<-data_random_forest[1:(nrow(data_random_forest)/2),] # further subset to speed up calculations and understand the code
nrow(data_random_forest)

data_random_forest<-data_random_forest %>% 
  distinct()

nrow(data_random_forest)
unique(data_random_forest$presence)
unique(data_random_forest$gear)
unique(data_random_forest$length_category)
unique(data_random_forest$sst_c_mean)

head(data_random_forest)

tic()
model_forest <- randomForest(formula = presence ~ gear + length_category + sst_c_mean, data=data_random_forest, type = "classification") # classification use Gini - logistic regression
toc() 
model_forest
predict(model_forest, type="prob")
