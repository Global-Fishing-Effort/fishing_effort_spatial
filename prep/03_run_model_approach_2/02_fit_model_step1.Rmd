...
## read in environmental variables
  ...
  env_data <- spatial_data %>%
    ...
    mutate(eez_region_world_bank_7 = ifelse(ISO_SOV1 %in% c("High seas", "Land"), "high_seas", eez_region_world_bank_7)) %>% 
...
foreach(flag = flags, .packages = c("dplyr", "glue", "stats", "tictoc", "tidyr")) %dopar% {
  ...
  if(length(unique(full_data$gear)) == 1 | length(unique(full_data$length_category)) == 1 ){ # skipping any that only have one categorical variable since we can't model these? 
  return(NULL)
