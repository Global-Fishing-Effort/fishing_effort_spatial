...
We apply a variable selection methdolody of...
...
```{r}
test <- qs::qread(file.path(rdsi_dir, "prep/random_forest/stage_1_models/pruning/stage_1_rf_train_BGR_25.qs"))

## ok so things started to go wrong at BGR. After than, each pruned model saved is much larger in size than the full model, which doesn't make sense. I will need to rerun. Perhaps as just a for loop instead of a foreach loop. I think the parallelization screwed things up. Or something within the while loop messed things up. Because it saved all of the full models fine (the models with 25 variables)
...
flags <- setdiff(flags, c("ALB", "BGR", "AGO")) # remove those we've already finished

flags <- flags[35:length(flags)]
flags <- "ZAF"

# Set up parallel backend
#### NOTE: THIS ISN'T WORKING; for some reason it gets hung up when saving the new rf model and saves it as realllllly large and only saves one per flag, which isn't correct. I think it might have to do with the while loops I've included. In theory it should work though...
# num_cores <- 3 # Use one less than the total available cores
# cl <- makeCluster(num_cores)
# registerDoParallel(cl)
 ...
 while (TRUE) {
  ...
  auc_values <- c(auc_values, auc_pr_i)
    ...
    if (length(auc_values) > 1) {
        # model_improvement <- (rmse_values[length(rmse_values) - 1] - rmse_values[length(rmse_values)]) / rmse_values[length(rmse_values) - 1] # RMSE is the opposite of AUC; i.e., a lower RMSE is better whereas a higher AUC is better
