...
gfw_year <- read.csv(file.path(rdsi_raw_dir, glue("global_fishing_watch/apparent_fishing_hours_mmsi/v2_aggregated/all_effort_{yr}.csv"))) %>%
...
gfw_year_aggregated <- gfw_year %>%
  ...
  group_by(x_1, y_1, flag_fin, vessel_class_gfw, length_m_gfw) %>%
...
# sum(gfw_year$fishing_hours) - sum(gfw_fix_props$fishing_hours) # 2012 is off by ~86k hours??
