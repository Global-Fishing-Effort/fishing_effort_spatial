...
```{r}
test_lengths <- gfw_vessel_registry_fix %>%
...
foreach(yr = 2013:2024, .packages = c(
  "qs", "here", "dplyr", "tidyr", "glue", "terra", "countrycode"
    ...
    #  yr = 2012
gfw_year_raw <- qread(file.path(rdsi_raw_dir, glue("global_fishing_watch/apparent_fishing_hours_mmsi/v3_aggregated/all_effort_{yr}.qs")))

gfw_year <- gfw_year_raw %>%
  dplyr::select(x = cell_ll_lon, y = cell_ll_lat,  mmsi, hours, fishing_hours) %>%
  left_join(gfw_vessel_registry_fix, by = "mmsi") %>% # cool - it worked for 2012
  distinct() %>% 
...
# make sure total amount lines up
# sum(gfw_year$fishing_hours) == sum(gfw_year_raw$fishing_hours) # TRUE 
...
gfw_year_aggregated <- gfw_year %>%
  ...
  group_by(x_05, y_05, flag_fin, length_category, gear) %>%
...
# sum(gfw_fix_props$fishing_hours) # 7638755
# sum(gfw_year_raw$fishing_hours) # 7723319
# # losing some hours. I believe this is because there are some trawling countries reported in GFW that are not reported in Rousseau, so we filter out those cells. 

qs::qsave(gfw_fix_props, file.path(rdsi_dir, glue("prep/gfw_props/deg_half/all_effort_gear_length_props_{yr}.qs")))
...
test_one <- qread(file.path(rdsi_dir, glue("prep/gfw_props/deg_one/all_effort_gear_length_props_2012.qs")))

sum(test_5$fishing_hours) # 7638755

sum(test_10$fishing_hours) # 7638976

gfw_year <- qread(file.path(rdsi_raw_dir, glue("global_fishing_watch/apparent_fishing_hours_mmsi/v3_aggregated/all_effort_2012.qs")))

sum(gfw_year$fishing_hours) # 7723319

## ok so something is obviously going wrong with the aggregation...
```
